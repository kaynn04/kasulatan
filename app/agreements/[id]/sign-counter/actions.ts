...
export async function signAgreement(
    ...
    } else if (typedSignature.localeCompare(counterparty.fullName, undefined, { sensitivity: "accent" }) !== 0) {
    ...
    const signed = await prisma.$transaction(async (tx) => {
        const claimed = await tx.agreement.updateMany({
            where: {
                id: agreementId,
                status: "DRAFT",
                parties: { some: { id: counterparty.id, userId: session.id } },
            },
            data: { status: "SIGNED_BY_COUNTERPARTY" },
        });
    ...
    if (!signed) {
        return { success: false, errors: { general: "This agreement cannot be signed or was already signed." } };
    }
